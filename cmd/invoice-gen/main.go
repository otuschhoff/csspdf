...
func runIntro(args []string) int {
	cmd := flag.NewFlagSet("intro", flag.ContinueOnError)
	cmd.SetOutput(os.Stderr)

	var (
		outputPath  = cmd.String("o", "output/intro.pdf", "Output PDF path")
		companyPath = cmd.String("company", "configs/myCompany.json", "Company JSON path")
		pageWidth   = cmd.Float64("page-width", invoice.DocWidth, "Page width in points")
		pageHeight  = cmd.Float64("page-height", invoice.DocHeight, "Page height in points")
		pageCount   = cmd.Int("pages", 2, "Number of pages to generate")
	...
	cmd.Usage = func() {
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  invoice-gen intro [options]")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Options:")
		cmd.PrintDefaults()
	}
	...
	if err := invoice.RenderIntroPDF(*outputPath, *companyPath, *pageWidth, *pageHeight, *pageCount); err != nil {
		fmt.Fprintf(os.Stderr, "Error rendering intro PDF: %v\n", err)
		return 1
	}
